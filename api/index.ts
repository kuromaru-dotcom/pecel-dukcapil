...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (method === 'POST' && path === '/api/auth/login') {
      ...
      if (!user || user.password !== password) return res.status(401).json({ error: "Username atau password salah" });
    ...
    if (method === 'POST' && path === '/api/users') {
      const [newUser] = await db.insert(users).values(req.body).returning({id:users.id,username:users.username,role:users.role});
