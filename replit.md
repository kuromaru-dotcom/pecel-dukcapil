...
The frontend is built with React and TypeScript, using Vite as the build tool. Wouter handles client-side routing, and TanStack Query manages server state. UI components leverage Radix UI primitives and shadcn/ui (New York style), styled with Tailwind CSS and Class Variance Authority (CVA). The design system features a fresh green and white color palette (primary green: 145 70% 40% on pure white backgrounds), Plus Jakarta Sans typeface, and a mobile-first responsive approach with WCAG AA accessibility compliance. State management primarily uses React Query for server state and local React state for UI interactions.
...
- ✅ Professional print-ready receipt/bukti penerimaan dokumen with official government header
- ✅ QR Code generator with application URL (https://pecel-dukcapil.vercel.app/) for easy access
...
The application features a **fresh green and white color theme** (primary green: HSL 145 70% 40% on pure white backgrounds), Plus Jakarta Sans font, rounded-2xl cards, rounded-full buttons, and redesigned status badges with WCAG AA accessibility compliance for all text colors. Mobile optimization ensures all interactive elements meet minimum touch targets and layouts are responsive. Key features include:
