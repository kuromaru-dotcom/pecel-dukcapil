...
import { Plus, Users, ChevronLeft, ChevronRight, LayoutDashboard, Download } from "lucide-react";
import { useLocation } from "wouter";

export default function Home() {
  const [, setLocation] = useLocation();
  
  const [userRole, setUserRole] = useState<'public' | 'cs' | 'operator' | 'superadmin'>('public');
  ...
  // Filter documents by all column filters and sort newest-first
  const filteredDocuments = useMemo(() => {
    return documents
      .filter(doc => {
        const matchesNama = !filterNama || doc.nama.toLowerCase().includes(filterNama.toLowerCase());
        const matchesTanggal = !filterTanggal || doc.tanggal.includes(filterTanggal);
        const matchesRegister = !filterRegister || doc.nomorRegister.toLowerCase().includes(filterRegister.toLowerCase());
        ...
        const matchesNamaCS = !filterNamaCS || (doc.namaCS && doc.namaCS.toLowerCase().includes(filterNamaCS.toLowerCase()));
        const matchesNamaOperator = !filterNamaOperator || (doc.namaOperator && doc.namaOperator.toLowerCase().includes(filterNamaOperator.toLowerCase()));
        
        return matchesNama && matchesTanggal && matchesRegister && matchesAlamat && matchesJenisDokumen && matchesStatus && matchesNamaCS && matchesNamaOperator;
      })
      .sort((a, b) => b.id - a.id); // Sort by ID descending (newest first)
  }, [documents, filterNama, filterTanggal, filterRegister, filterAlamat, filterJenisDokumen, filterStatus, filterNamaCS, filterNamaOperator]);
