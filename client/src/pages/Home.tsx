...
export default function Home() {
  ...
  const [filterNamaCS, setFilterNamaCS] = useState<string>('all');
  const [filterNamaOperator, setFilterNamaOperator] = useState<string>('all');
  ...
  // Fetch CS users for dropdown filter
  const { data: csUsers = [] } = useQuery<{ username: string; role: string }[]>({
    queryKey: ['/api/users/by-role/cs'],
  });

  // Fetch Operator users for dropdown filter
  const { data: operatorUsers = [] } = useQuery<{ username: string; role: string }[]>({
    queryKey: ['/api/users/by-role/operator'],
  });
  ...
  const filteredDocuments = useMemo(() => {
    return documents
      .filter(doc => {
        ...
        const matchesNamaCS = filterNamaCS === 'all' || (doc.namaCS && doc.namaCS === filterNamaCS);
        const matchesNamaOperator = filterNamaOperator === 'all' || (doc.namaOperator && doc.namaOperator === filterNamaOperator);
  ...
  }, [documents, debouncedFilterNama, dateRange, debouncedFilterRegister, filterAlamat, filterJenisDokumen, filterStatus, filterNamaCS, filterNamaOperator]);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 md:px-6 py-6 md:py-8">
        ...
        {/* Column Filters */}
        <div className="mb-6 p-4 md:p-6 bg-card border border rounded-2xl shadow-sm">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            ...
            <div>
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="outline"
                    className="w-full h-10 rounded-xl border justify-start text-left font-normal"
            ...
            <div>
              ...
              <Select value={filterJenisDokumen} onValueChange={(val) => {
                ...
                <SelectTrigger className="h-10 rounded-xl border" data-testid="select-filter-jenis">
            ...
            <div>
              ...
              <Select value={filterStatus} onValueChange={(val) => {
                ...
                <SelectTrigger className="h-10 rounded-xl border" data-testid="select-filter-status">
                  <SelectValue placeholder="Semua" />
                </SelectTrigger>
                <SelectContent>
              ...
              <>
                <div>
                  ...
                  <Select value={filterAlamat} onValueChange={(val) => {
                    ...
                    <SelectTrigger className="h-10 rounded-xl border" data-testid="select-filter-alamat">
                      <SelectValue placeholder="Semua" />
                    </SelectTrigger>
                    <SelectContent>
                ...
                <div>
                  ...
                  <Select value={filterNamaCS} onValueChange={(val) => {
                    setFilterNamaCS(val);
                    setCurrentPage(1);
                  }}>
                    <SelectTrigger className="h-10 rounded-xl border" data-testid="select-filter-cs">
                ...
                <div>
                  ...
                  <Select value={filterNamaOperator} onValueChange={(val) => {
                    setFilterNamaOperator(val);
                    setCurrentPage(1);
                  }}>
                    <SelectTrigger className="h-10 rounded-xl border" data-testid="select-filter-operator">
